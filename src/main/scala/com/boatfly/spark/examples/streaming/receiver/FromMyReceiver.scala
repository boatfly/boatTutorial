...
object FromMyReceiver {
  def main(args: Array[String]): Unit = {
    ...
    val selfReceiverDStream: ReceiverInputDStream[String] = streamingContext.receiverStream(new MyReceiver("localhost", 9999))

    //将采集的数据进行分解（扁平化）
    val wordDStream: DStream[String] = selfReceiverDStream.flatMap(line => line.split(" "))
